...
///	jQuery.post('http://DebugMonitor/carjs/delete_log.jsp');
	...
	setTimeout = function(ptr, time){
		if(ptr.name!=null && ptr.name!=""){
			...
			$$logEventEntryFns($CAR_EVENT_SETTIMEOUTDEFFN,ptr.name,false);
		}else{
			stoutMap[$$counter] = ptr.toString();
			$$logEventEntryFns($CAR_EVENT_SETTIMEOUTANONFN,ptr.toString(),true);
		}
	...
	Math.random = function(){
		var val = $$oldRand();
		$$randCache += val + ",";
		$(document).find("#randStorage").append(val + ",");
		return val;
	}
...
function $$logEventEntryFns(style,fnname,anon){
	var logEntry = {Type:"fn",count:$$counter,DE:0,fnName:fnname,Anon:anon};
	var logEntryArr = JSON.parse(localStorage.getItem("UserId"+"_"+$$ssID));
	logEntryArr[$$counter++]=logEntry;
	localStorage.setItem("UserId"+"_"+$$ssID,JSON.stringify(logEntryArr));
}
...
function sendCompleteLogs(){
	...
	var randvuffer = "";
	/*for(var i=0;i<$$STORERANDID;i++){
		randvuffer+=$.jStorage.get("RandId_"+i);
	}*/
